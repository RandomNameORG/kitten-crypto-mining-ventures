      ...
      <div className="list">
        {snapshot.mercs.length === 0 ? (
          ...
          snapshot.mercs.map((merc) => (
            <article key={merc.instance_id} className="row item-row">
              <img className="item-icon" src={mercIconSrc(merc.def_id)} alt="" loading="lazy" />
        ...
        {snapshot.merc_defs.map((def) => (
          <article key={def.id} className="row item-row">
            <img className="item-icon" src={mercIconSrc(def.id)} alt="" loading="lazy" />
            <div className="item-content">
              <div className="row-head">
