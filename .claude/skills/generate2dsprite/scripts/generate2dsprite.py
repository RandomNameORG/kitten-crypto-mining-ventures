#!/usr/bin/env python3
"""Build sprite prompts, generate raw sheets via OpenRouter, and postprocess locally."""
...
def _openrouter_call(payload: dict, api_key: str, timeout: int = 240) -> dict:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "HTTP-Referer": "https://github.com/local/agent-sprite-forge",
            "X-Title": "agent-sprite-forge",
        },
        method="POST",
    )
    try:
        ...
        raise SystemExit(f"OpenRouter HTTP {e.code}: {body}") from e


def _decode_image_url(url: str) -> tuple[str, bytes]:
...
def _extract_first_image(resp: dict) -> tuple[str, bytes] | None:
        ...
        if isinstance(content, list):
            for part in content:
                ...
                if t in ("image_url", "image"):
                    ...
                    if url:
                        return _decode_image_url(url)
                if t in ("output_image", "image_base64"):
                    b64 = part.get("image") or part.get("data") or part.get("b64_json")
                    if b64:
                        return "png", base64.b64decode(b64)
    return None


def cmd_generate(args: argparse.Namespace) -> None:
    """Call OpenRouter image model and save the raw sheet to --out."""
    ...
    if args.size or args.quality:
        img_opts: dict = {}
        if args.size:
            img_opts["size"] = args.size
        if args.quality:
            img_opts["quality"] = args.quality
    ...
    if not api_key:
        raise SystemExit("ERROR: OPENROUTER_API_KEY is not set in the environment.")

    print(f"[generate] requesting {args.model} -> {out_path}", file=sys.stderr)
    resp = _openrouter_call(payload, api_key)
    image = _extract_first_image(resp)
    if not image:
        debug_path = out_path.with_suffix(".response.json")
        debug_path.write_text(json.dumps(resp, indent=2), encoding="utf-8")
        raise SystemExit(f"No image returned. Raw response saved to {debug_path}")

    ext, data = image
    if out_path.suffix.lower().lstrip(".") != ext and out_path.suffix:
        # Honor the requested suffix; if model returned non-PNG, normalise via Pillow.
        from io import BytesIO
        Image.open(BytesIO(data)).convert("RGBA").save(out_path)
    else:
    ...
    if args.write_prompt:
        ...
        print(f"[generate] prompt -> {prompt_path}", file=sys.stderr)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    generate_parser = subparsers.add_parser(
        "generate",
        help="Generate a raw sprite sheet via OpenRouter (saves PNG to --out).",
    )
    generate_parser.add_argument("--prompt", help="Inline art prompt.")
    generate_parser.add_argument("--prompt-file", type=Path, help="Read prompt from file.")
    generate_parser.add_argument("--out", required=True, type=Path, help="Output PNG path.")
    generate_parser.add_argument("--model", default=DEFAULT_OPENROUTER_MODEL,
                                 help=f"OpenRouter model id (default: {DEFAULT_OPENROUTER_MODEL}).")
    generate_parser.add_argument("--size", help="Image size, e.g. 1024x1024.")
    generate_parser.add_argument("--quality", choices=["low", "medium", "high"],
                                 help="Image quality hint.")
    generate_parser.add_argument("--write-prompt", action="store_true",
                                 help="Also write <out>.prompt.txt next to the PNG.")
